...
export const marketStatus = pgEnum('market_status', [
  'draft', 'pending_review', 'open', 'suspended', 'closed', 'resolving', 'resolved', 'cancelled', 'archived',
]);
export const settlementStatus = pgEnum('settlement_status', [
  'pending', 'detected', 'reviewing', 'calculating', 'posting', 'completed', 'failed', 'reversed',
]);
...
}, (table) => [uniqueIndex('provider_events_identity_uidx').on(table.provider, table.providerEventId)]);

export const providerMarkets = pgTable('provider_markets', {
  id: uuid('id').defaultRandom().primaryKey(),
...
export const marketPriceSnapshots = pgTable('market_price_snapshots', {
  id: uuid('id').defaultRandom().primaryKey(),
  marketId: uuid('market_id').notNull().references(() => markets.id),
  outcomeId: uuid('outcome_id').notNull().references(() => marketOutcomes.id),
...
}, (table) => [index('market_price_snapshots_lookup_idx').on(table.marketId, table.outcomeId, table.capturedAt)]);
...
export const ledgerAccounts = pgTable('ledger_accounts', {
  ...
  status: varchar('status', { length: 32 }).notNull().default('active'),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
}, (table) => [uniqueIndex('ledger_accounts_owner_asset_uidx').on(table.ownerType, table.ownerId, table.assetCode)]);
